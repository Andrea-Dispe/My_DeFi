...
export async function getPoolTOkenPerBlock(param = {}) {
    ...
    console.log("getPoolTOkenPerBlock: " + poolAlpacaPerBlock);

    return poolAlpacaPerBlock;
}

export async function getPoolTokenDaily(param = {}) {
    const fairLaunch = getWeb3FairLaunchContract();
    ...
    const poolTokenDaily = alpacaPerBlock * poolInfo.allocPoint * 24 * 60 * 60 / 3 / totalAllocPoint; // 24*60*60/3：24小时出块数量
    // console.log("getPoolTOkenPerBlock: " + poolTokenDaily);
    return poolTokenDaily;
}
`   `
const options = {
