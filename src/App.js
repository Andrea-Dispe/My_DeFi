...
import { getPoolHuskyDaily } from './services/FairlaunchService';
...
function App() {
  ...
  const [poolHuskyDaily, setPoolHuskyDaily] = useState([]);
  ...
  const stakingData = stakedValue.map((value, index) => {
    return { ...value, huskyDaily: poolHuskyDaily[index] };
  });
